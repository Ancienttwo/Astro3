...
export async function getCurrentUnifiedUser(request?: Request): Promise<UnifiedUser | null> {
  try {
    ...
    if (typeof window === 'undefined' && request) {
      ...
      // 首先尝试验证是否为我们的自定义JWT token (Web3用户)
      // 只在服务器端执行JWT验证
      if (typeof window === 'undefined') {
        try {
          console.log('🔍 开始JWT验证流程...')
          
          // 动态导入jsonwebtoken，只在服务器端使用
          const jwt = eval('require')('jsonwebtoken')
          const jwtSecret = process.env.JWT_SECRET
          
          console.log('🔍 JWT验证环境检查:', {
          ...
          if (!jwtSecret) {
            console.log('❌ JWT_SECRET环境变量未设置')
            throw new Error('JWT_SECRET not configured')
          }
          
          if (jwtSecret.length < 32) {
            console.log('❌ JWT_SECRET长度不足（需要至少32字符）')
          ...
          if (jwtPayload && jwtPayload.userId) {
            console.log('🔍 JWT payload有效，查找对应的Web3用户...')
            
            // 从数据库查找Web3用户
            const { data: web3User, error: web3Error } = await supabase
              .from('users')
              .select('*')
            ...
            console.log('🔍 数据库查询结果:', {
              ...
              errorCode: web3Error?.code,
              userDetails: web3User ? {
                id: web3User.id,
                email: web3User.email,
                auth_type: web3User.auth_type,
                wallet_address: web3User.wallet_address
              } : null
            })
            
            if (web3User && !web3Error) {
              console.log('✅ 服务器端通过JWT找到Web3用户:', {
                email: web3User.email,
                username: web3User.username,
                auth_type: web3User.auth_type,
                wallet_address: web3User.wallet_address
              })
              return {
                ...
                created_at: web3User.created_at,
                updated_at: web3User.updated_at
              }
            } else {
              console.log('❌ JWT有效但找不到对应的Web3用户:', {
                errorMessage: web3Error?.message,
                errorCode: web3Error?.code,
                searchedUserId: jwtPayload.userId
              })
              return null
            }
          } else {
            console.log('❌ JWT payload无效或缺少userId:', jwtPayload)
            return null
          }
        } catch (e) {
          console.log('🔍 JWT验证失败，尝试Supabase认证:', {
            errorName: (e as Error).name,
            errorMessage: (e as Error).message,
            isTokenMalformed: (e as Error).message.includes('malformed'),
            isTokenExpired: (e as Error).message.includes('expired'),
            isSignatureInvalid: (e as Error).message.includes('signature')
          })
        }
      }
