...
import type { ZiWeiCompleteChart, ZiWeiChartInput, BirthInfo, PalaceInfo, StarInfo, MajorPeriodInfo } from '../complete-chart-types';
...
export class ZiweiCalculatorSingleton {
  private static instance: ZiweiCalculatorSingleton;
  private cacheStats: CacheStats = { hits: 0, misses: 0, totalCalculations: 0 };
  private calculationCache = new Map<string, ZiWeiCompleteChart>();
   ...
   * 计算完整紫微命盘
   */
  async calculateComplete(input: ZiWeiChartInput): Promise<ZiWeiCompleteChart> {
    // 生成缓存键
   ...
   * 生成缓存键
   */
  private generateCacheKey(input: ZiWeiChartInput): string {
   ...
   * 执行实际计算 (临时模拟实现)
   */
  private performCalculation(input: ZiWeiChartInput): ZiWeiCompleteChart {
    // 使用 tyme4ts 通过 data-conversion 统一入口获取八字与农历信息
    // 为避免对外部类型的强依赖，这里按 any 引入 SolarTime
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { SolarTime }: any = require('tyme4ts')
    ...
    if (typeof input.isLunar === 'boolean') {
      (birthInfo.lunar as any).isLunar = input.isLunar
    }
    if (typeof input.isLeapMonth === 'boolean') {
      (birthInfo.lunar as any).isLeapMonth = input.isLeapMonth
    }
    ...
    // 五行局
    const feb = calculateFiveElementsBureauDetail(
      baziParams.yearStem,
    ...
    const auxPos = calculateAuxiliaryStarPositions(
      baziParams.lunarMonth,
      baziParams.lunarDay,
      baziParams.timeZhiIndex,
      baziParams.yearStem,
    ...
    const malPos = calculateMaleficStarPositions(
      baziParams.lunarMonth,
      baziParams.timeZhiIndex,
      baziParams.yearStem,
      baziParams.yearBranch
    ...
    const majorPeriodsAll = calculateMajorPeriods(
      startAgeByBureau,
      input.year,
      lifePalaceIndex,
      isClockwise
    ...
    const startIndex = ((): number => {
      ...
      const yb = branches.indexOf(baziParams.yearBranch as any)
      return startPositions[yb] ?? 4
    })()
    const male = input.gender === 'male'
    const minorLimitByAge: number[] = []
    ...
    for (let i = 0; i < 12; i++) {
      ...
      // 该宫内的星曜
      const starsAt = (mp: Map<string, number[]>) => {
        const res: string[] = []
        mp.forEach((positions, name) => { if (positions.includes(i)) res.push(name) })
        return res
      }
      ...
      // 亮度映射
      const toStarObjects = (names: string[]): StarInfo[] => names.map((name) => ({
        name,
        bright: ((): string => {
        ...
        sihua: ((): string => {
          if (birthSihua.A === name) return 'A'
          if (birthSihua.B === name) return 'B'
          if (birthSihua.C === name) return 'C'
          if (birthSihua.D === name) return 'D'
          return ''
        })(),
        // 自化标记：离心 xA/xB/xC/xD（本宫宫干），向心 iA/iB/iC/iD（对宫宫干）
        self_sihua: ((): string => {
          const oppositePalaceIndex = (i + 6) % 12
          const stemForIndex = (index: number): string => {
            const wuhu: Record<string, string> = { '甲': '丙', '己': '丙', '乙': '戊', '庚': '戊', '丙': '庚', '辛': '庚', '丁': '壬', '壬': '壬', '戊': '甲', '癸': '甲' }
            const yinStem = wuhu[baziParams.yearStem] || '甲'
            const stems = ['甲','乙','丙','丁','戊','己','庚','辛','壬','癸']
            const start = stems.indexOf(yinStem)
          ...
          const tsForStar = selfTs.filter((t: any) => t.star === name)
          if (tsForStar.length === 0) return ''
          const mapLetter = (t: any) => {
            if (t.type.includes('禄')) return 'A'
            if (t.type.includes('权')) return 'B'
            if (t.type.includes('科')) return 'C'
            if (t.type.includes('忌')) return 'D'
            return ''
          }
          const tokens = tsForStar.map((t: any) => (t.direction === 'outward' ? 'x' : 'i') + mapLetter(t)).filter((z: string) => /^(i|x)[ABCD]$/.test(z))
          return tokens.join(' ')
        })(),
      }))

      const mainStars: StarInfo[] = toStarObjects(mainStarsNames)
      const auxiliaryStars: StarInfo[] = toStarObjects(auxStarsNames)
      // 杂曜 = 煞星 + 桃花星（不参与四化，自化），亮度统一“平”
      const minorStars: StarInfo[] = [...maleficNames, ...romanceNames].map(name => ({ name, bright: '平' }))

      // 本宫大运（若匹配不到，设为0占位，但不使用任何随机/伪造数据）
      const mp = majorPeriodsAll.find(p => p.palaceIndex === i)
      const majorPeriod: MajorPeriodInfo = mp
      ...
      const yearBranchIndex = CORE_BRANCHES.indexOf(baziParams.yearBranch as any)
      const fleetingYears: number[] = []
      for (let age = 1; age <= 120; age++) {
        const idx = (yearBranchIndex + age - 1) % 12
        if (idx === i) fleetingYears.push(age)
      }
      const minorPeriod = (() => {
        const ages: number[] = []
        for (let age = 1; age <= 120; age++) if (minorLimitByAge[age - 1] === i) ages.push(age)
        return ages
      })()
    ...
    const complete: ZiWeiCompleteChart = {
      birthInfo,
      ...
      lifeMaster: calculateMasters(birthInfo.lunar.yearBranch, (CORE_BRANCHES[lifePalaceIndex] || CORE_BRANCHES[lifePalaceIndex % 12]) as any).lifeMaster,
      bodyMaster: calculateMasters(birthInfo.lunar.yearBranch, (CORE_BRANCHES[lifePalaceIndex] || CORE_BRANCHES[lifePalaceIndex % 12]) as any).bodyMaster,
 ...
 * 获取计算器实例的便捷函数
 */
export function getZiweiCalculator(): ZiweiCalculatorSingleton {
