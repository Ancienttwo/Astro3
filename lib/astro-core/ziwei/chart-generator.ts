 ...
 * 计算所有星曜位置
 */
function calculateAllStarPositions(baziParams: BaZiParams): IStarPositions {
  const ziweiPosition = calculateZiweiPosition(baziParams.yearStem, baziParams.lunarDay)
...
export function generateCompleteZiWeiChart(baziParams: BaZiParams): IZiWeiCompleteChart {
  try {
    ...
    const starPositions = calculateAllStarPositions(baziParams)
