 ...
 * @version 2.0.0
 */
import { STAR_BRIGHTNESS_TABLE, BRIGHTNESS_LEVEL_MAP } from '../constants';
/**
 * 获取星曜在指定宫位的亮度数值
 ...
 */
export function getStarBrightness(starName, palaceIndex) {
 ...
 */
export function getStarBrightnessLevel(starName, palaceIndex) {
    const brightnessArray = STAR_BRIGHTNESS_TABLE[starName];
    if (!brightnessArray) {
        throw new Error(`[Brightness] Unknown star: ${starName}`);
    }
 ...
 * @returns 是否入庙
 */
export function isStarInTemple(starName, palaceIndex) {
 ...
 * @returns 是否旺盛
 */
export function isStarProsperous(starName, palaceIndex) {
    const brightnessLevel = getStarBrightnessLevel(starName, palaceIndex);
 ...
 * @returns 是否失陷
 */
export function isStarTrapped(starName, palaceIndex) {
    const brightnessLevel = getStarBrightnessLevel(starName, palaceIndex);
 ...
 * @returns 亮度评分
 */
export function getStarBrightnessScore(starName, palaceIndex) {
    return getStarBrightness(starName, palaceIndex);
}
//# sourceMappingURL=brightness-calculations.js.map