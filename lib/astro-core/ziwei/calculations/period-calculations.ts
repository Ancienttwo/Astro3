...
export function calculateMajorPeriodStartAge(
  bureau: string, 
  yearStem: string | HeavenlyStem, 
  gender: 'male' | 'female'
): number {
  const [yangAge, yinAge] = MAJOR_PERIOD_START_AGE[bureau] || [6, 5]
  
  // 判断阴阳
  const stemIndex = STEMS.indexOf(yearStem as HeavenlyStem)
 ...
 * @returns 太岁地支索引
 */
export function getTaisuiBranch(year: number): number {
  // 太岁地支 = (年份 - 4) % 12
  return (year - 4) % 12
}
