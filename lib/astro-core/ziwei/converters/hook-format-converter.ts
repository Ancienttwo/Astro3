 ...
 * 转换出生信息为Hook格式
 */
export function convertBirthInfoToHook(birthInfo: BirthInfo): HookBirthInfo {
  const base: any = {
    // 公历信息
  ...
  if (typeof (birthInfo.lunar as any).isLeapMonth === 'boolean') {
    base.isLeapMonth = (birthInfo.lunar as any).isLeapMonth as boolean
  }
  return base as HookBirthInfo
}
...
function getBranchIndex(branchName: string): number {
  const index = BRANCH_NAMES.indexOf(branchName as BranchName);
 ...
 * 转换星曜信息为Hook格式
 */
function convertStarToHook(star: StarInfo): HookStarInfo {
  const types: string[] = [];

  // 处理生年四化（A/B/C/D）
  if (star.sihua && /^[ABCD]$/.test(star.sihua)) types.push(star.sihua)

  // 处理自化（x*/i*），支持空格或逗号分隔的多个标记
  if (star.self_sihua) {
    const parts = String(star.self_sihua).split(/[\s,]+/).filter(Boolean)
    for (const p of parts) {
      if (/^(i|x)[ABCD]$/.test(p)) types.push(p)
    }
  }

  const base: HookStarInfo = {
    name: star.name,
 ...
 * 转换大运信息为Hook格式
 */
function convertMajorPeriodToHook(period: MajorPeriodInfo): HookMajorPeriod {
  return {
 ...
 * 转换五行局信息为Hook格式
 */
function convertFiveElementsBureauToHook(bureau: FiveElementsBureau): HookFiveElementsBureau {
  return {
    name: bureau.name,
    局数: bureau.number
  };
...
function convertPalaceToHook(
  ...
  const res: any = {
    ...
    fleetingYears: (palace as any).fleetingYears ?? generateFleetingYears(5 + branchIndex),
    majorPeriod: convertMajorPeriodToHook(palace.majorPeriod),
    // 小限：必须使用核心算法产出的年龄分布，严禁用流年等价替代
    minorPeriod: (palace as any).minorPeriod ?? []
  }
  // 兼容：显式提供 ringIndex（等于 branchIndex）
  res.ringIndex = branchIndex
  return res as HookPalaceInfo;
}

/**
 * 转换完整紫微命盘为Hook格式
 */
export function convertZiWeiChartToHook(chart: ZiWeiCompleteChart): ZiWeiHookChart {
  const hookChart: Partial<ZiWeiHookChart> = {
    // 基础信息
    ...
    命宫: (() => {
      for (const p of Object.values(chart.palaces)) {
        if ((p as any)?.palaceName === '命宫') return (p as any).branch
      }
      return '子'
    })(),
    身宫: (() => {
      ...
      const hourZhiIndex = (chart.birthInfo.lunar.hourBranch && typeof chart.birthInfo.lunar.hourBranch === 'string')
        ? branches.indexOf(chart.birthInfo.lunar.hourBranch as any)
        : (chart.birthInfo as any).timeZhiIndex ?? 0
      const monthPalace = (2 + month - 1 + 12) % 12
      const bodyIndex = (monthPalace + hourZhiIndex) % 12
      const bodyBranch = branches[bodyIndex] || '子'
      const entry = Object.values(chart.palaces).find((p: any) => p.branch === bodyBranch) as any
    ...
    斗君: (() => {
      const v: unknown = (chart as any).doujun
      const branches = BRANCH_NAMES as readonly string[]
      if (typeof v === 'number') return branches[v] || '子'
      const n = Number(v)
      if (Number.isFinite(n)) return branches[n] || '子'
      if (typeof v === 'string' && branches.includes(v as any)) return v as string
      return '子'
    })(),
  ...
  BRANCH_NAMES.forEach((branchName, index) => {
    ...
    if (palace) {
      (hookChart as any)[branchName] = convertPalaceToHook(palace, branchName, index);
    } else {
      // 创建空宫位
      (hookChart as any)[branchName] = createEmptyPalace(branchName, index);
    }
  });
  
  return hookChart as ZiWeiHookChart;
}

/**
 * 根据地支查找宫位
 */
function findPalaceByBranch(palaces: { [key: string]: PalaceInfo }, branchName: string): PalaceInfo | null {
 ...
 * 创建空宫位
 */
function createEmptyPalace(branchName: BranchName, branchIndex: number): HookPalaceInfo {
  return {
    branch: branchName,
    branchIndex: branchIndex,
    stem: "", // 需要根据实际算法计算
