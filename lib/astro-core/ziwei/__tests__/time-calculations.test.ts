...
describe('时间计算功能测试', () => {
  ...
  describe('getCurrentMajorPeriod - 大运期计算', () => {
    it('应该正确计算阳男的大运期', () => {
      ...
      const majorPeriod = getCurrentMajorPeriod(
        birthDate,
        targetDate,
        'male',
        '庚',      // 1990年为庚午年，庚为阳干
    ...
    it('应该正确计算阴女的大运期', () => {
      const birthDate = new Date('1991-01-01')
  ...
  describe('getFleetingYear - 流年计算', () => {
    it('应该正确计算流年干支', () => {
      ...
      // 2020年（庚子年）
      const fleeting2020 = getFleetingYear(birthDate, 2020)
      ...
      // 2024年（甲辰年）
      const fleeting2024 = getFleetingYear(birthDate, 2024)
    ...
    it('流年宫位索引应该与地支索引对应', () => {
      ...
      // 子年
      const yearZi = getFleetingYear(birthDate, 2020)
      expect(yearZi.palaceIndex).toBe(0) // 子的索引为0
      
      // 午年
      const yearWu = getFleetingYear(birthDate, 2014)
  ...
  describe('getFleetingMonth - 流月计算', () => {
    it('应该正确计算流月干支', () => {
      ...
      // 2024年甲辰年的正月应该是丙寅月
      const month1 = getFleetingMonth(birthDate, 2024, 1)
      expect(month1.stem).toBe('丙')
      expect(month1.branch).toBe('寅')
      expect(month1.ganZhi).toBe('丙寅')
      
      // 2024年甲辰年的五月应该是庚午月
      const month5 = getFleetingMonth(birthDate, 2024, 5)
    ...
    it('应该遵循五虎遁规则', () => {
      ...
      // 甲己年起丙寅月
      const jiaNian = getFleetingMonth(birthDate, 2024, 1) // 2024甲辰年
      expect(jiaNian.stem).toBe('丙')
      
      // 乙庚年起戊寅月
      const gengNian = getFleetingMonth(birthDate, 2020, 1) // 2020庚子年
  ...
  describe('getMinorLimit - 小限计算', () => {
    it('男命小限应该顺行', () => {
      ...
      const minor1 = getMinorLimit(birthDate, targetDate1, 'male', '午')
      const minor2 = getMinorLimit(birthDate, targetDate2, 'male', '午')
    ...
    it('女命小限应该逆行', () => {
      ...
      const minor1 = getMinorLimit(birthDate, targetDate1, 'female', '午')
      const minor2 = getMinorLimit(birthDate, targetDate2, 'female', '午')
    ...
    it('应该根据年支确定起始位置', () => {
      ...
      // 子年生人
      const ziYear = getMinorLimit(birthDate, targetDate, 'male', '子')
      expect(ziYear.palaceIndex).toBe(3) // 从辰宫起（索引4-1=3）
      
      // 丑年生人
      const chouYear = getMinorLimit(birthDate, targetDate, 'male', '丑')
