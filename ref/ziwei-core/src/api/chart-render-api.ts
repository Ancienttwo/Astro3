 ...
 */
function isHookPalaceInfo(value: unknown): value is HookPalaceInfo {
  return value !== null && 
         typeof value === 'object' && 
         'branch' in value && 
 ...
 */
function getPalaceInfo(hookChart: ZiWeiHookChart, branch: BranchName): HookPalaceInfo {
...
class ChartRenderCache {
  private cache = new Map<string, RenderDataCache>();
  private readonly maxSize = 50;
  private readonly ttl = 30 * 60 * 1000; // 30分钟

  generateCacheKey(input: ChartRenderInput): string {
  ...
  private generateHookFingerprint(hookChart: ZiWeiHookChart): string {
    const key = {
  ...
  get(cacheKey: string): RenderDataCache | null {
  ...
  set(cacheKey: string, data: Partial<RenderDataCache>): void {
    // LRU清理
 ...
 * 将Hook格式的星曜信息转换为渲染格式
 */
function convertHookStarToRenderStar(star: HookStarInfo): RenderStarInfo {
  const types: ('主星' | '辅星' | '煞星' | '小星' | '四化')[] = [];
  let sihua: '禄' | '权' | '科' | '忌' | undefined;
  let color: 'primary' | 'secondary' | 'warning' | 'danger' | 'success' = 'primary';

  // 解析星曜类型标记
  if (star.type) {
    star.type.forEach(t => {
      if (t === 'D') types.push('主星');
      if (t === 'A') types.push('辅星');
      if (t === 'S') types.push('煞星');
      if (t === 'M') types.push('小星');
      
      // 四化标记解析
      if (t === 'iA' || t === 'xA') { sihua = '禄'; types.push('四化'); color = 'success'; }
      if (t === 'iB' || t === 'xB') { sihua = '权'; types.push('四化'); color = 'warning'; }
      if (t === 'iC' || t === 'xC') { sihua = '科'; types.push('四化'); color = 'primary'; }
      if (t === 'iD' || t === 'xD') { sihua = '忌'; types.push('四化'); color = 'danger'; }
    });
  }
  ...
  const base: RenderStarInfo = {
    name: star.name,
    types,
    color,
    highlight: !!sihua
  }
  if (sihua) base.sihua = sihua
  if (star.brightness) {
    // 只有在有亮度值时才添加该可选属性，避免 exactOptionalPropertyTypes 误报
    base.brightness = star.brightness as '庙' | '旺' | '得' | '利' | '平' | '陷'
  }
  return base
}
 ...
 * 将Hook格式的宫位信息转换为渲染格式
 */
function convertHookPalaceToRenderPalace(
  branchName: string, 
  ...
  if (mainStarCount >= 2 || sihuaCount >= 2) strength = 'strong';
  else if (mainStarCount === 0 && sihuaCount === 0) strength = 'weak';

  const obj: RenderPalaceInfo = {
    branch: branchName as any,
    branchIndex: palace.branchIndex,
    ringIndex: palace.branchIndex,
    stem: palace.stem,
    palaceName: palace.palaceName as any,
    stars,
 ...
 * 生成中宫渲染信息
 */
function generateCenterInfo(hookChart: ZiWeiHookChart): RenderCenterInfo {
  return {
 ...
 * 生成四化连线
 */
function generateSihuaLines(palaces: RenderPalaceInfo[]): RenderSihuaLine[] {
  const lines: RenderSihuaLine[] = [];
 ...
 * 为Web平台生成渲染数据
 */
export function renderPalaceForWeb(input: ChartRenderInput): WebRenderData {
  ...
  const palaces = branches.map(branch => 
    convertHookPalaceToRenderPalace(branch, getPalaceInfo(hookChart, branch as BranchName), options)
  );
 ...
 * 为Native平台生成渲染数据
 */
export function renderPalaceForNative(input: ChartRenderInput): NativeRenderData {
  const cacheKey = renderCache.generateCacheKey(input);
  const cached = renderCache.get(cacheKey);
  ...
  // Native特有的尺寸和主题信息
  const nativeData: NativeRenderData = {
    palaces,
    centerInfo, 
    sihuaLines,
    dimensions: {
 ...
 * 通用渲染数据生成器
 */
export function generateChartRenderData(input: ChartRenderInput) {
 ...
 * 批量生成渲染数据 (Web + Native)
 */
export function generateBothPlatformRenderData(input: ChartRenderInput) {
  return {
 ...
 * 生成十二宫网格位置
 */
function generateGridPositions(): Array<{ row: number; col: number }> {
  return [
    { row: 3, col: 2 }, // 子
    { row: 3, col: 1 }, // 丑  
    { row: 3, col: 0 }, // 寅
