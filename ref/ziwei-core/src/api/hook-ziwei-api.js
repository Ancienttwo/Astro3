 ...
 */
export async function generateZiWeiHookChart(input) {
    try {
        // 转换输入格式
 ...
 * 验证和增强Hook格式命盘数据
 */
function validateAndEnhanceHookChart(chart, input) {
    // 计算小限（按年支三合库对宫起点；男顺、女逆）用于占位数据
    const computeMinorPeriodAges = (branchIndex) => {
        // 起点地支：根据出生年支的三合库对宫
        const branches = BRANCH_NAMES;
        const yearBranch = chart.birthInfo.yearBranch;
        const ybIndex = branches.indexOf(yearBranch);
 ...
 */
export async function generateBatchZiWeiHookCharts(inputs) {
    const results = [];
 ...
 * 验证闰月
 */
function validateLeapMonth(_input) {
    // 这里需要使用tyme4ts库来验证闰月
    // 简单实现，实际应该调用农历库
    return true;
}
...
//# sourceMappingURL=hook-ziwei-api.js.map