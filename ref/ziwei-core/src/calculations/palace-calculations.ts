 ...
 */
export function calculateLaiyinPalace(yearStem: string | HeavenlyStem, lifePalaceIndex: number): string {
  // 五虎遁表：年干 -> 寅宫起始天干
  ...
  if (!startStem) return (PALACE_NAMES[0] as string) // 默认返回命宫
  
  // 从寅宫(index=2)开始，按五虎遁推算各宫天干
  const startStemIndex = STEMS.indexOf(startStem as HeavenlyStem)
  ...
  for (let i = 0; i < 12; i++) {
    ...
    const palaceStem = (STEMS[palaceStemIndex] || STEMS[(palaceStemIndex + 10) % 10]) as string
    
    // 找到与年干相同的宫位
    if (palaceStem === yearStem) {
      // 计算该宫位相对于命宫的位置，返回对应的宫名
      const relativeIndex = (palaceIndex - lifePalaceIndex + 12) % 12
      return (PALACE_NAMES[relativeIndex] as string)
    }
  }
  
  return (PALACE_NAMES[0] as string) // 默认返回命宫
}
