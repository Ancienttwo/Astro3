...
export function calculateZiweiPosition(bureauCode: string, day: number): number {
  ...
  // 将地支名称转换为索引
  const positionIndex = BRANCHES.indexOf(positionBranch as EarthlyBranch)
 ...
 * @returns 主星位置Map
 */
export function calculateMainStarPositions(ziweiPos: number, tianfuPos: number): Map<string, number[]> {
...
export function calculateMaleficStarPositions(
  ...
  const huoxingBaseBranch = HUOXING_BASE_MAP[BRANCHES[yearBranchIndex] as EarthlyBranch]
  ...
  const lingBaseBranch: EarthlyBranch = (yb === '寅' || yb === '午' || yb === '戌' ? '卯' : '戌') as EarthlyBranch
...
export function calculateMinorStarPositions(
  _yearBranch: string | EarthlyBranch,
  _monthBranch: string | EarthlyBranch,
  _dayBranch: string | EarthlyBranch,
  _hourBranch: string | EarthlyBranch
): Map<string, number[]> {
  const positions = new Map<string, number[]>()
