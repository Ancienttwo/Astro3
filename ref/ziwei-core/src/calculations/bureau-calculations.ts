...
export interface FiveElementsBureauDetail {
  name: string      // 中文名称，如 "火六局"
  局数: number      // 局数，如 6
  element: string   // 五行，如 "火"
  code: string      // 代码，如 "fire_6"
}
...
export function calculateFiveElementsBureauDetail(
  ...
  const elementMap: Record<string, string> = { '水': 'water', '木': 'wood', '金': 'metal', '土': 'earth', '火': 'fire' }
  const elementCn = nameCn.charAt(0)
  const numberCn = nameCn.charAt(1) // "二"/"三"/...
  const numberMap: Record<string, number> = { '一': 1, '二': 2, '三': 3, '四': 4, '五': 5, '六': 6 }
