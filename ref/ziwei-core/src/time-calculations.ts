...
export function getCurrentMajorPeriod(
  ...
  return {
    ...
    endAge: periodEndAge,
    palaceIndex,
    palaceName: getPalaceName(palaceIndex, chartContext.lifePalaceIndex) ?? '',
    stem,
    branch,
    startYear,
    endYear
 ...
 * @param year 目标年份
 */
export function getFleetingYear(
  birthDate: Date,
  year: number,
  lifePalaceIndex = 0
): IFleetingYearInfo {
  ...
  // 简化版宫位名称（实际应该根据命盘布局）
  // 使用命宫索引旋转宫名，确保视觉排序保持逆时针语义
  const palaceName = getPalaceName(palaceIndex, lifePalaceIndex)
...
export function getFleetingMonth(
  ...
  return {
    ...
    palaceIndex,
    // 与 getPalaceName 保持一致，避免流月列表出现顺时针错位
    palaceName: getPalaceName(palaceIndex, lifePalaceIndex)
  }
}
...
export function getMinorLimit(
  chartContext: Pick<IChartContext, 'birthDate' | 'gender' | 'yearBranch'> & { lifePalaceIndex?: number },
  targetDate: Date
): IMinorLimitInfo
export function getMinorLimit(
  birthDate: Date,
...
export function getMinorLimit(
  arg1: Pick<IChartContext, 'birthDate' | 'gender' | 'yearBranch'> & { lifePalaceIndex?: number } | Date,
  arg2: Date,
  arg3?: 'male' | 'female',
  arg4?: string,
  arg5?: number
): IMinorLimitInfo {
  let chartContext: Pick<IChartContext, 'birthDate' | 'gender' | 'yearBranch'> & { lifePalaceIndex?: number }
  let targetDate: Date
  ...
  if (arg1 instanceof Date) {
    chartContext = {
      birthDate: arg1,
      gender: arg3 ?? 'male',
      yearBranch: (arg4 ?? '子') as Branch,
      lifePalaceIndex: arg5
    }
    targetDate = arg2
  } else {
    chartContext = arg1
  ...
  const lifePalaceIndex = typeof chartContext.lifePalaceIndex === 'number'
    ? chartContext.lifePalaceIndex
    : 0
...
export function getComprehensiveTimeInfo(
  ...
  return {
    ...
    fleetingYear: getFleetingYear(chartContext.birthDate, targetYear, chartContext.lifePalaceIndex),
    fleetingMonth: getFleetingMonth(chartContext.birthDate, targetYear, targetMonth, chartContext.lifePalaceIndex),
