...
export const PALACE_INDEX_TO_NAME: Record<number, PalaceIdentifier> = PALACE_ORDER.reduce(
  (acc, palace, idx) => {
    acc[idx + 1] = palace
    return acc
  },
  {} as Record<number, PalaceIdentifier>,
)

export const MARRIAGE_KEY_PALACES: readonly PalaceIdentifier[] = [
  '命宫',
  '夫妻宫',
  '子女宫',
  '财帛宫',
...
export const MARRIAGE_INNER_PALACES: readonly PalaceIdentifier[] = [
  '命宫',
  '夫妻宫',
  '财帛宫',
  '疾厄宫',
...
export const MARRIAGE_PALACE_ROLES: MarriagePalaceRole[] = [
...
export const MARRIAGE_LETTER_MEANINGS: Record<MarriageLetter, string> = {
...
export const MARRIAGE_COMBINATION_MEANINGS: Record<MarriageCombinationKey, MarriageCombinationMeaning> = {
  AB: {
    key: 'AB',
    label: '禄权组合',
...
export const MARRIAGE_STAR_NOTES = {
  ...
  noble:
    '贵人星对婚姻的冲击较大，尤以左辅、右弼、化科显现时，容易二度婚姻或外缘干扰。',
  nobleRepetition:
    '左辅、右弼象征再一次或二度缘分，落夫妻宫时常见婚前抉择难或二婚、小三课题。',
}
...
export const MARRIAGE_CHILD_PALACE_GUIDE =
  '子女宫行戊干飞 AC 入命（或子田线）时，代表后天桃花触发，为有缘又有情的良性桃花走势。'
