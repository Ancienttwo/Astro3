...
const RISK_CHAIN_PALACES: readonly PalaceIdentifier[] = ['交友宫', '疾厄宫', '田宅宫']
const MITIGATION_PALACES: readonly PalaceIdentifier[] = ['命宫', '疾厄宫', '福德宫', '交友宫', '田宅宫']
const SYMPTOM_PALACES: readonly PalaceIdentifier[] = ['疾厄宫', '命宫', '福德宫']

function createEmptyBuckets(): MarkerBuckets {
  const initial: Record<HealthLetter, string[]> = { A: [], B: [], C: [], D: [] }
  return {
    origin: { ...initial },
    inward: { ...initial },
    outward: { ...initial },
...
function collectAllStars(palace: HookPalaceInfo | null): HookStarInfo[] {
  if (!palace) return []
  const mainStars = (palace as any)['mainStars&sihuaStars'] as HookStarInfo[] | undefined
  const auxiliaryStars = (palace as any)['auxiliaryStars&sihuaStars'] as HookStarInfo[] | undefined
  const minorStars = (palace as any).minorStars as HookStarInfo[] | undefined
  return [
    ...(mainStars ?? []),
    ...(auxiliaryStars ?? []),
    ...(minorStars ?? []),
...
function collectMarkerBuckets(stars: HookStarInfo[]): MarkerBuckets {
  ...
  stars.forEach((star) => {
    ...
    types.forEach((tag) => {
      if (!tag) return
      if (/^[ABCD]$/.test(tag)) {
        const letter = tag as HealthLetter
        buckets.origin[letter].push(star.name)
      } else if (/^i[ABCD]$/.test(tag)) {
        const letter = tag.slice(1) as HealthLetter
...
function hasLetter(summary: PalaceMarkerSummary | undefined, letter: HealthLetter): boolean {
  if (!summary) return false
  return (
    summary.markers.origin[letter].length > 0 ||
    summary.markers.inward[letter].length > 0 ||
    summary.markers.outward[letter].length > 0
  )
}

function formatLetterDetail(summary: PalaceMarkerSummary, letter: HealthLetter): string {
  ...
  if (origin.length) sections.push(`生年：${origin.join('、')}`)
  if (inward.length) sections.push(`向心：${inward.join('、')}`)
  if (outward.length) sections.push(`离心：${outward.join('、')}`)
  return sections.join('；')
}

function evaluateRiskFindings(markers: Record<PalaceIdentifier, PalaceMarkerSummary>): HealthRiskFinding[] {
  const findings: HealthRiskFinding[] = []
...
function evaluateMitigation(markers: Record<PalaceIdentifier, PalaceMarkerSummary>): HealthMitigationHint[] {
  const hints: HealthMitigationHint[] = []
  MITIGATION_PALACES.forEach((palace) => {
    ...
    if (origin.length === 0 && inward.length === 0 && outward.length === 0) return
    const sources: Partial<Record<MarkerSource, string[]>> = {}
    if (origin.length) sources.origin = origin
    if (inward.length) sources.inward = inward
    if (outward.length) sources.outward = outward
    const segments: string[] = []
    if (origin.length) segments.push(`生年科：${origin.join('、')}`)
    if (inward.length) segments.push(`向心科：${inward.join('、')}`)
    if (outward.length) segments.push(`离心科：${outward.join('、')}`)
    hints.push({
      palace,
      letter: 'C',
      sources,
...
function gatherSymptomHighlights(chart: ZiWeiHookChart): SymptomHighlight[] {
  const highlights: SymptomHighlight[] = []
...
export function analyzeHealthFromHook(chart: ZiWeiHookChart): HealthAnalysisResult {
  const markers: Record<PalaceIdentifier, PalaceMarkerSummary> = {} as Record<PalaceIdentifier, PalaceMarkerSummary>
