...
import {
  ...
  buildWealthInputFromChartData,
  PALACE_ORDER,
  PALACE_NAME_ALIASES,
  analyzeWealthStructure,
} from './wealth'
import { COMBINATION_RULES, type CombinationRule } from './fortune-rules'
...
export interface InnatePatternAnalysis {
  ...
  notes: string[]
  referencePalace: { name: PalaceIdentifier | null; index: number | null }
}
...
function getPalaceNameByIndex(index?: number | null): PalaceIdentifier | null {
  if (!index || index < 1 || index > PALACE_ORDER.length) return null
  const palace = PALACE_ORDER[index - 1]
  return palace ?? null
}

function filterValidIndices(indices: number[] = []): number[] {
  return indices.filter((value): value is number => Number.isInteger(value) && value > 0 && value <= PALACE_ORDER.length)
}

function extractPalaceNames(indices: number[]): PalaceIdentifier[] {
  return filterValidIndices(indices)
    .map((idx) => getPalaceNameByIndex(idx))
    .filter((name): name is PalaceIdentifier => Boolean(name))
}

function isCombinationKey(value: string | undefined | null): value is CombinationRule['key'] {
...
function getPalaceIndex(name: PalaceIdentifier | null): number | null {
  if (!name) return null
  const idx = PALACE_ORDER.indexOf(name)
  return idx >= 0 ? idx + 1 : null
}

function analyzeOrigin(originPalace: PalaceIdentifier | null): OriginAnalysis {
  ...
  const mode: OriginMode = isInner ? 'self_reliant' : 'supported'
...
function collectBirthTransformPositions(input: WealthAnalysisInput): Record<'A' | 'B' | 'C' | 'D', number[]> {
  const positions: Record<'A' | 'B' | 'C' | 'D', number[]> = {
    A: [],
    B: [],
    C: [],
    D: [],
...
function analyzeInnatePattern(birthPositions: Record<'A' | 'B' | 'C' | 'D', number[]>): InnatePatternAnalysis {
  ...
  const orientation: PatternOrientation = YIN_INDEXES.has(primaryIndex) ? 'wealth_oriented' : 'status_oriented'
  ...
  const recommendation: 'focus_wealth' | 'focus_status' | 'stabilize_first' = isBroken || isDamaged
    ? 'stabilize_first'
...
function describeTransform(letter: 'A' | 'B' | 'C' | 'D', indices: number[]): string {
  const validIndices = filterValidIndices(indices)
  if (!validIndices.length) return `${letter} 未在命盘中标注生年四化。`
  ...
  const tags: string[] = []
  if (bodyTag) tags.push('体宫')
  if (useTag) tags.push('用宫')
  const tagText = tags.length ? `（${tags.join(' / ')}）` : ''
  return `${letter} 落在 ${palaceText}${tagText}`
}

function buildTransformDetails(
  birthPositions: Record<'A' | 'B' | 'C' | 'D', number[]>,
  comboByIndex: Map<number, CombinationRule['key'][]>,
): TransformDetail[] {
...
function computeOpposite(index: number): number[] {
  if (!index) return []
  return [((index + 6 - 1) % 12) + 1]
}

function computeNeighbors(index: number): number[] {
  if (!index) return []
...
function buildPriorityMatrix(indexes: number[]): PriorityMatrix {
  if (!indexes.length) {
    return {
      stages: [
...
function buildPriority(birthPositions: Record<'A' | 'B' | 'C' | 'D', number[]>): Record<'A' | 'B' | 'C' | 'D', PriorityMatrix> {
  return {
...
function extractOriginFromHook(chart: ZiWeiHookChart): PalaceIdentifier | null {
  return normalizePalaceName(chart?.来因宫 as string)
}

export function analyzePatternFromWealthInput(
  input: WealthAnalysisInput,
  originPalace: PalaceIdentifier | null,
  ...
  const comboByIndex = new Map<number, CombinationRule['key'][]>(
    wealthStructure.palaceCombinations.map((item) => {
...
export function analyzePatternFromHookChart(chart: ZiWeiHookChart): PatternAnalysisResult {
...
export function analyzePatternFromChartData(
  chart: MinimalChartDataInput,
  options?: { originPalace?: string | null },
): PatternAnalysisResult {
