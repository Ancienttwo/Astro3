...
export const PALACE_ORDER: PalaceIdentifier[] = [
  '命宫',
  '兄弟宫',
  '夫妻宫',
  '子女宫',
...
const ROLE_MATRIX: Record<string, readonly number[]> = {
  内宫: [1, 5, 6, 9, 10, 11],
  外宫: [2, 3, 4, 7, 8, 12],
  阳宫: [1, 3, 5, 7, 9, 11],
  阴宫: [2, 4, 6, 8, 10, 12],
...
const FOUNDATION_COMBINATIONS: Record<string, string> = {
...
export const PALACE_NAME_ALIASES: Record<string, PalaceIdentifier> = {
  命宫: '命宫',
  命: '命宫',
  兄弟宫: '兄弟宫',
  兄弟: '兄弟宫',
...
function normalizePalaceName(name: string | undefined): PalaceIdentifier | null {
  if (!name) return null
...
function collectMarkers(stars: WealthStarInput[] | undefined): MarkerSet {
  const result: MarkerSet = {
    base: new Set(),
    inward: new Set(),
    outward: new Set(),
  }
  if (!stars) return result
  stars.forEach((star) => {
...
function detectCombinations(markers: MarkerSet): string[] {
  const combos: string[] = []
  const base = markers.base
  const has = (code: 'A' | 'B' | 'C' | 'D') => base.has(code) || markers.inward.has(code) || markers.outward.has(code)
  const combinations: Array<[string, ('A' | 'B' | 'C' | 'D')[]]> = [
    ['AB', ['A', 'B']],
    ['AC', ['A', 'C']],
    ['BC', ['B', 'C']],
    ['AD', ['A', 'D']],
...
export function analyzeWealthStructure(input: WealthAnalysisInput): WealthAnalysisResult {
  const palaceRoles: PalaceRoleInsight[] = []
  const palaceCombinations: PalaceCombinationInsight[] = []
  ...
  input.palaces.forEach((palace) => {
    ...
    if (combos.length) {
      const markerList = Array.from(new Set([
        ...
        ...Array.from(markers.inward).map((code) => `i${code}` as const),
        ...Array.from(markers.outward).map((code) => `x${code}` as const),
      ]))
...
export function buildWealthInputFromHook(chart: ZiWeiHookChart): WealthAnalysisInput {
  const palaces: WealthPalaceInput[] = []
  PALACE_ORDER.forEach((palaceName, idx) => {
    const branchKey = getBranchByIndex(idx)
    const palaceInfo = chart[branchKey]
    if (!isHookPalaceInfo(palaceInfo)) return
    const stars: WealthStarInput[] = []
    const collectStar = (star: HookStarInfo) => {
    ...
    const auxiliaryStars = palaceInfo['auxiliaryStars&sihuaStars'] as HookStarInfo[] | undefined
    const minorStars = palaceInfo.minorStars as HookStarInfo[] | undefined
    ...
    const palaceEntry: WealthPalaceInput = {
      index: idx + 1,
      name: palaceName,
...
function getBranchByIndex(index: number): keyof ZiWeiHookChart {
  const branches: Array<keyof ZiWeiHookChart> = ['子', '丑', '寅', '卯', '辰', '巳', '午', '未', '申', '酉', '戌', '亥']
  return branches[index] ?? '子'
}

function isHookPalaceInfo(value: unknown): value is HookPalaceInfo {
  return Boolean(value && typeof value === 'object' && 'palaceName' in (value as Record<string, unknown>))
}

export function buildWealthInputFromChartData(chart: MinimalChartDataInput): WealthAnalysisInput {
  const palaces: WealthPalaceInput[] = []
  chart.palaces.forEach((palace) => {
    ...
    palace.stars?.forEach((star) => {
      const markers: string[] = []
      if (star.sihuaOrigin) markers.push(star.sihuaOrigin)
      if (star.sihuaInward) markers.push(`i${star.sihuaInward}`)
      if (star.sihuaOutward) markers.push(`x${star.sihuaOutward}`)
      stars.push({ name: star.name, markers })
    })
    const palaceEntry: WealthPalaceInput = {
      index: index + 1,
