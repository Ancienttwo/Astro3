...
interface ZiweiReasoningAgentProps {
  ziweiData: ZiweiData
  userId: string
  selectedRecord?: any
...
export function ZiweiReasoningAgent({ 
  ziweiData, 
  userId, 
  selectedRecord,
  updateRecord,
  ...
  const streaming = useStreamingAnalysis({
    ...
    onComplete: (fullContent) => {
      console.log('✅ 紫微推理分析完成，总内容长度:', fullContent.length)
      
      // 保存结果到本地记录（兼容离线浏览）
      if (selectedRecord && updateRecord) {
        const cacheKey = generateCacheKey();
        const analysisKey = `ziwei_reasoning_${cacheKey}`;
        
        const aiAnalyses = selectedRecord.aiAnalyses || {};
        aiAnalyses[analysisKey] = {
          result: fullContent,
          timestamp: new Date().toISOString(),
          type: 'ziwei_reasoning',
          powered_by: 'DIFY流式分析',
  ...
  const generateCacheKey = useCallback(() => {
    const ziweiString = `${ziweiData.name}_${ziweiData.birthday}`;
    return `${ziweiString}_${ziweiData.gender}`;
  }, [ziweiData]);
  ...
  // 启动流式紫微推理分析
  const startZiweiReasoningAnalysis = useCallback(async () => {
    if (!ziweiData.name || !ziweiData.birthday) {
      console.error('请先输入完整的紫微斗数信息');
      return;
    }
    ...
    try {
      ...
      const taskData = {
        ...
        status: 'pending',
        input_data: {
          cacheKey: `ziwei_reasoning_${Date.now()}`,
          ziweiData: {
            ...ziweiData,
            username: ziweiData.name, // 确保有username字段
          },
          analysisType: 'ziwei_reasoning'
        },
      ...
      console.log('✅ 紫微推理流式任务创建成功:', task.id)
      setTaskId(task.id)

    } catch (error) {
      console.error('❌ 紫微推理分析失败:', error)
      setTaskError(error instanceof Error ? error.message : '未知错误')
    } finally {
      setIsCreatingTask(false)
    }
  }, [ziweiData, generateCacheKey]);

  // 监听分析状态变化并通知父组件
  useEffect(() => {
    if (onStatusChange) {
      onStatusChange({
        isLoading: streaming.isStreaming || isCreatingTask,
        isCompleted: streaming.isCompleted,
        isFailed: !!streaming.error || !!taskError,
        elapsedTime: streaming.elapsedTime,
        formattedElapsedTime: streaming.formattedElapsedTime,
        error: streaming.error || taskError
      });
    }
  }, [
    ...
    streaming.error,
    streaming.elapsedTime,
    streaming.formattedElapsedTime,
    isCreatingTask,
    taskError,
    onStatusChange
  ]);
    ...
    <>
      <Card className="w-full">
        ...
        <CardContent className="space-y-4">
          <div className="p-4 bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-lg">
                            <p className="text-sm text-purple-800 dark:text-yellow-400">
              🔮 <strong>紫微推理</strong>：基于紫微斗数十二宫和十四主星配置，深度推理命主的性格特质、事业方向、感情模式和人生格局。
            </p>
          </div>
          ...
          {existingAnalysis && !streaming.isStreaming && !isCreatingTask && (
            <div className="p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
              <div className="text-blue-800 dark:text-blue-200 text-sm flex items-center gap-2">
                ...
                已有紫微推理报告 (生成于 {new Date(existingAnalysis.timestamp).toLocaleString()})
              </div>
            </div>
          )}
              ...
              {streaming.content && (
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    {streaming.isCompleted && (
                      <span className="text-xs bg-green-100 dark:bg-green-900 text-green-700 dark:text-green-300 px-2 py-1 rounded">已完成</span>
                    )}
                  </div>
                  <div 
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              {existingAnalysis && !streaming.isStreaming && !isCreatingTask && (
                <Button 
                  ...
                  className="flex items-center gap-2 bg-gradient-to-r from-purple-500 to-indigo-600 hover:from-purple-600 hover:to-indigo-700 text-white border-0"
                >
              ...
              {!streaming.isStreaming && !isCreatingTask && (
                <Button 
                  onClick={startZiweiReasoningAnalysis}
                  className="flex items-center gap-2 bg-gradient-to-r from-purple-500 to-indigo-600 hover:from-purple-600 hover:to-indigo-700 text-white border-0"
                  disabled={!ziweiData.name || !ziweiData.birthday}
                  variant={existingAnalysis ? "outline" : "default"}
                >
                  <Target className="h-4 w-4" />
                  {existingAnalysis ? "重新推理" : "开始紫微推理"}
                </Button>
              )}
      ...
      <Dialog open={showResultDialog} onOpenChange={setShowResultDialog}>
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-xl font-bold text-purple-600 dark:text-purple-400 flex items-center gap-2">
