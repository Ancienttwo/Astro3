      ...
      <Card>
        <CardContent className="p-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="font-medium">{tAuth('walletConnector.connectTitle')}</span>
          </div>
          <p className="text-sm text-muted-foreground mb-4">{tAuth('walletConnector.connectSubtitle')}</p>
          <p className="text-xs text-muted-foreground mb-2">{tAuth('walletConnector.connectorsHeading')}</p>
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          <span className="flex items-center gap-2">
            <Wallet className="w-5 h-5" /> {tAuth('walletConnector.connectedTitle')}
          </span>
          {onBsc ? (
            <Badge className="bg-green-500 text-white">
              <CheckCircle2 className="w-3 h-3 mr-1" /> {tAuth('walletConnector.correctNetworkBadge')}
            </Badge>
          ) : (
            <Badge variant="destructive">
              <AlertCircle className="w-3 h-3 mr-1" /> {tAuth('walletConnector.wrongNetworkBadge')}
            </Badge>
          )}
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        <div className="flex items-center justify-between">
          <span className="text-sm text-gray-600">{tAuth('walletConnector.addressLabel')}:</span>
        ...
        <div className="flex items-center justify-between">
          <span className="text-sm text-gray-600">{tAuth('walletConnector.balanceLabel')}:</span>
          <span className="font-medium">{bal?.formatted ?? "0"} {bal?.symbol ?? "BNB"}</span>
        </div>

        {!onBsc && bscMainnet && (
          <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
            <p className="text-sm text-yellow-700 mb-2">{tAuth('walletConnector.switchPrompt')}</p>
            <Button variant="outline" size="sm" onClick={() => switchChain({ chainId: bscMainnet.id })} disabled={isSwitching} className="w-full">
              {isSwitching ? <Loader2 className="h-4 w-4 animate-spin" /> : tAuth('walletConnector.switchButton')}
            </Button>
          </div>
        )}
