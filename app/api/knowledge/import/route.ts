import { NextRequest, NextResponse } from 'next/server'
import { createRouteHandlerClient } from '@supabase/auth-helpers-nextjs'
import { cookies } from 'next/headers'

export async function POST(request: NextRequest) {
  try {
    if (process.env.ENABLE_FORTUNE_AGENT !== 'true') {
      return NextResponse.json({ success: false, error: 'Fortune Agent disabled' }, { status: 403 })
    }

    const supabase = createRouteHandlerClient({ cookies })
    const { data: { user } } = await supabase.auth.getUser()

    // 简单的管理员检查（基于 app_metadata.role）
    const role = (user as any)?.app_metadata?.role
    if (!user || role !== 'admin') {
      return NextResponse.json({ success: false, error: 'Forbidden' }, { status: 403 })
    }

    const { category, content, metadata } = await request.json()
    if (!category || !content) {
      return NextResponse.json({ success: false, error: '缺少必要参数' }, { status: 400 })
    }

    const { importKnowledge } = await (eval('import')('@/lib/services/knowledge-service'))
    const result = await importKnowledge(category, content, metadata || {})
    return NextResponse.json({ success: true, data: result })
  } catch (error) {
    console.error('Knowledge import error:', error)
    return NextResponse.json({ success: false, error: '导入失败' }, { status: 500 })
  }
}

