...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`获取用户档案: ${user.email} (${user.id}) - Auth Type: ${user.auth_type || 'supabase'}`)
    ...
    if (!profile) {
      ...
      if (createError) {
        ...
        return NextResponse.json({
          ...
          user_info: {
            auth_type: user.auth_type || 'supabase',
...
export async function PUT(request: NextRequest) {
  try {
    ...
    console.log(`更新用户档案: ${user.email} (${user.id}) - Auth Type: ${user.auth_type || 'supabase'}，日期是否变更: ${is_date_changed}，性别是否变更: ${is_gender_changed}`)
